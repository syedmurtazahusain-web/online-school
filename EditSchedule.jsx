...
function EditSchedule() {
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      try {
        const data = await get(`http://localhost:5000/teacher/schedule/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await put(`http://localhost:5000/teacher/schedule/${id}`, formData);
    ...
    return (
      <div style={{fontFamily: 'Arial, sans-serif', padding: '20px', background: '#f8f9fa', minHeight: '100vh'}}>
        <div style={{textAlign: 'center'}}>
          <div style={{fontSize: '24px'}}>⏳ Loading class...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{fontFamily: 'Arial, sans-serif', padding: '20px', background: '#f8f9fa', minHeight: '100vh'}}>
      <div style={{maxWidth: '600px', margin: '0 auto', background: 'white', padding: '30px', borderRadius: '10px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)'}}>
        <h1 style={{margin: '0 0 20px 0', color: '#333'}}>✏️ Edit Class</h1>
